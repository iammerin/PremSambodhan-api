...
from .serializers import RegisterSerializer, LoginSerializer, UserDataSerializer, MessageSerializer, MessagingSerializer, UpdateProfileSerializers
...
class RegisterAPIView(APIView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        if serializer.is_valid():
            ...
            print(confirm_password,'confirm password')
...
class LoginAPIView(APIView):
    def post(self, request, *args, **kwargs):
        ...
        if serializer.is_valid():
            ...
            toret = {}
            if UserProfile.objects.filter(user=query).exists():
                toret['update_status'] = True
            else:
                toret['update_status'] = False
...
class MessagingAPIView(APIView):
    def get(self, request):
        ...
        for each in user:
            toret = {}
            toret['id'] = each['id']
            toret['username'] = each['username']
            toret['image'] = []
            for image in UserProfile.objects.filter(user_id=each['id']).values('profile_image'):
                print(image)
                toret['image'] = image['profile_image']
            toList.append(toret)
        return JsonResponse({"code": 200, "status": "success",
    ...
    def post(self, request, *args, **kwargs):
        ...
        if serializer.is_valid():
            ...
            reciever_id = serializer.data['reciever_id']
            Message.objects.create(sender=user, receiver_id=reciever_id,
                                   message=message)
...
class UpdateProfileAPIView(APIView):
    def post(self, request):
        ...
        try:
            ...
            return Response({'code': 404, 'status': 'failure', 'message': 'Invalid Credentials'})
        user_id = usid.user_id
        serializer = UpdateProfileSerializers(data=request.data)
        if serializer.is_valid():
            print(request.META['HTTP_AUTHORIZATION'],'token')
            # print(serializer.data['full_name'],'serializer data')
            queryset = UserProfile.objects.filter(user_id=user_id)
            print(queryset)
            try:
                queryset.update(full_name=serializer.data['full_name'])
            except KeyError as e:
                print(e)
                pass
            
            try:
                queryset.update(address=serializer.data['address'])
            except KeyError:
                pass

            try:
                queryset.update(contact_number=serializer.data['contact_number'])
            except KeyError:
                pass

            try:
                queryset.update(instagram_id=serializer.data['instagram_id'])
            except KeyError:
                pass

            try:
                queryset.update(facebook_id=serializer.data['facebook_id'])
            except KeyError:
                pass

            try:
                queryset.update(gender=serializer.data['gender'])
            except KeyError:
                pass

            try:
                queryset.update(profile_image=serializer.data['profile_image'])
            except KeyError:
                pass
...
class GetProfileAPIView(APIView):
    def get(self, request):
        ...
        toret['contact_number']=queryset.contact_number
        toret['instagram_id']=queryset.instagram_id
        toret['facebook_id']=queryset.facebook_id
        toret['gender']=queryset.gender
        toret['profile_image']=queryset.profile_image.imageURL
        return JsonResponse({"code": 200, "status": "success",
                                "message": "Successfully Fetched",
                                 "details": toret})
